...
const CommandLine = Module("commandline", {
    ...
    _echoMultiline: function (str, highlightGroup) {
        ...
        this._lastMowOutput = xml({raw: ["<div class=\"ex-command-output\" style=\"white-space: nowrap\" highlight=", ">", "</div>"],cooked: ["<div class=\"ex-command-output\" style=\"white-space: nowrap\" highlight=", ">", "</div>"]}, [(highlightGroup), (template.maybeXML(str))]);
        ...
        liberator.callInMainThread(function () {
            ...
            if (single)
                ...
                if (this._messageBox.value == this._lastEcho)
                    this._echoMultiline(xml({raw: ["<span highlight=\"Message\">", "</span>"],cooked: ["<span highlight=\"Message\">", "</span>"]}, [(this._lastEcho)]),
    ...
    Completions: Class("Completions", {
        ...
        preview: function preview() {
            ...
            let node = util.xmlToDom(xml({raw: ["<span highlight=\"Preview\">", "</span>"],cooked: ["<span highlight=\"Preview\">", "</span>"]}, [(substring)]),
                document);
    ...
    echoArgumentToString: function (arg, useColor) {
        ...
        if (typeof arg == "string" && /\n/.test(arg))
            arg = xml({raw: ["<span highlight=\"CmdOutput\">", "</span>"],cooked: ["<span highlight=\"CmdOutput\">", "</span>"]}, [(arg)]);
            ...
            function () {
                ...
                /*if (commandline._messageHistory.length == 1) {
                    ...
                    let list = template.map2(xml, commandline._messageHistory.messages, function (message)
                        xml({raw: ["<div highlight=", ">", "</div>"],cooked: ["<div highlight=", ">", "</div>"]}, [(message.highlight + " Message"), (message.str)]));
...
const ItemList = Class("ItemList", {
    ...
    _init: function () {
        this._div = this._dom(
            xml({raw: ["<div class=\"ex-command-output\" highlight=\"Normal\" style=\"white-space: nowrap\">\n\
                <div highlight=\"Completions\" key=\"noCompletions\"><span highlight=\"Title\">No Completions</span></div>\n\
                <div key=\"completions\"/>\n\
                <div highlight=\"Completions\">\n\
                ", "\n\
                </div>\n\
            </div>"],cooked: ["<div class=\"ex-command-output\" highlight=\"Normal\" style=\"white-space: nowrap\">                <div highlight=\"Completions\" key=\"noCompletions\"><span highlight=\"Title\">No Completions</span></div>                <div key=\"completions\"/>                <div highlight=\"Completions\">                ", "                </div>            </div>"]}, [(
                    template.map2(xml, util.range(0, options["maxitems"] * 2), function (i)
                    xml({raw: ["<span highlight=\"CompItem\">\n\
                        <li highlight=\"NonText\">~</li>\n\
                    </span>"],cooked: ["<span highlight=\"CompItem\">                        <li highlight=\"NonText\">~</li>                    </span>"]}, []))
                )]), this._divNodes);
        ...
        this._items.contextList.forEach(function init_eachContext(context) {
            ...
            if (!context.items.length && !context.message && !context.incomplete)
                return;
            context.cache.nodes = [];
            this._dom(xml({raw: ["<div key=\"root\" highlight=\"CompGroup\">\n\
                    <div highlight=\"Completions\">\n\
                        ", "\n\
                    </div>\n\
                    <div key=\"message\" highlight=\"CompMsg\"/>\n\
                    <div key=\"items\" highlight=\"Completions\"/>\n\
                    <div key=\"waiting\" highlight=\"CompMsg\">", "</div>\n\
                </div>"],cooked: ["<div key=\"root\" highlight=\"CompGroup\">                    <div highlight=\"Completions\">                        ", "                    </div>                    <div key=\"message\" highlight=\"CompMsg\"/>                    <div key=\"items\" highlight=\"Completions\"/>                    <div key=\"waiting\" highlight=\"CompMsg\">", "</div>                </div>"]}, [( context.createRow(context.title || [], "CompTitle") ), (ItemList.WAITING_MESSAGE)]), context.cache.nodes);
